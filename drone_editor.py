...
─────────────────────────
Give it one or more drone videos (files and/or folders). For each clip it will:
...
You choose what each run edits:
  • One long clip on its own            → pass that one file.
  • Several clips merged into one video  → pass them all with --combine.
...
Usage:
    python drone_editor.py INPUT [INPUT ...] [options]

Examples:
    # Edit one 5-minute clip on its own
    python drone_editor.py flight.mp4

    # Edit ten short clips together into a single highlight reel
    python drone_editor.py clip1.mp4 clip2.mp4 clip3.mp4 --combine --music song.mp3

    # Combine everything in a folder into one video
...
# ── Stage 4: cut + reframe + concat (one or many sources) ────────────────────
def render_segments(inputs: list, segs: list, aspect: str, out_path: str,
                    music: str | None, keep_audio: bool) -> bool:
    """Render `segs` into a single video.

    `inputs` is a list of source files. `segs` is a list of (input_index,
    start, end). Each segment is trimmed, reframed to the target aspect, and
    the results are concatenated in the order given — so this handles both a
    single clip and several clips merged together.

    With `music`, the track replaces the audio (ideal for highlight reels of
    wind-noisy drone footage). With `keep_audio`, the original audio of each
    segment is carried through and joined instead.
    """
    if not segs:
        return False
    ...
    for i, (idx, s, e) in enumerate(segs):
        parts.append(f"[{idx}:v]trim=start={s:.3f}:end={e:.3f},"
                     f"setpts=PTS-STARTPTS,{rf}[v{i}]")
        vlabels.append(f"[v{i}]")
        if keep_audio and music is None:
            parts.append(f"[{idx}:a]atrim=start={s:.3f}:end={e:.3f},"
    ...
    n = len(segs)
    fc = ";".join(parts) + ";"
    fc += "".join(vlabels) + f"concat=n={n}:v=1:a=0[vout]"
    ...
    if music:
        ...
        cmd += ["-af", f"afade=t=out:st={max(0.0, total - 2):.2f}:d=2", "-shortest"]
...
# ── Build a clean, analyzed master for one source clip ───────────────────────
def build_master(src: str, out_dir: str, stabilize: bool):
    """Return (master_path, master_info, motion_buckets) or None on failure."""
    name = os.path.splitext(os.path.basename(src))[0]
    info = probe(src)
    if info["duration"] <= 0:
        print(f"  Could not read {os.path.basename(src)} — skipping.")
        return None
    ...
    if not make_clean_master(src, master, stabilize):
        print(f"  Could not prepare {os.path.basename(src)} — skipping.")
        return None

    minfo = probe(master)
    print("    Analyzing motion...")
    buckets = motion_buckets(master)
    return master, minfo, buckets


# ── Mode A: each clip edited on its own ──────────────────────────────────────
def process_single(src: str, out_dir: str, args, stabilize: bool, music: str | None):
    name = os.path.splitext(os.path.basename(src))[0]
    print(f"\n{'='*64}\n  {os.path.basename(src)}\n{'='*64}")

    built = build_master(src, out_dir, stabilize)
    if not built:
        return
    master, minfo, buckets = built
    ...
    print(f"    Trimmed edit: {sum(e - s for s, e in keep):.0f}s kept "
          f"({len(keep)} segment(s)) | highlight: ~{sum(e - s for s, e in high):.0f}s")
    ...
    for aspect in aspects:
        ...
        if render_segments([master], [(0, s, e) for s, e in high],
                           aspect, reel, music, keep_audio=False):
            print(f"      Saved: {reel}")

        if not args.no_full:
            full = os.path.join(out_dir, f"{name}_edit_{tag}.mp4")
            print(f"    Rendering trimmed edit ({tag})...")
            if render_segments([master], [(0, s, e) for s, e in keep],
                               aspect, full, None, keep_audio=minfo["has_audio"]):
                print(f"      Saved: {full}")
...
# ── Mode B: several clips merged into one video ──────────────────────────────
def process_combined(videos: list, out_dir: str, args, stabilize: bool,
                     music: str | None, base: str):
    print(f"\n{'='*64}\n  COMBINING {len(videos)} clip(s) → {base}\n{'='*64}")

    masters = []  # (master_path, minfo, buckets)
    for src in videos:
        built = build_master(src, out_dir, stabilize)
        if built:
            masters.append(built)
    if not masters:
        print("  No usable clips to combine.")
        return

    # Share the highlight target across clips so the reel stays ~highlight_secs.
    per = max(3.0, args.highlight_secs / len(masters))
    high_segs, keep_segs = [], []
    for idx, (m, minfo, buckets) in enumerate(masters):
        for s, e in highlight_segments(buckets, minfo["duration"], 1.0, per):
            high_segs.append((idx, s, e))
        for s, e in keep_segments(buckets, minfo["duration"], 1.0, args.trim_pct):
            keep_segs.append((idx, s, e))

    inputs = [m for m, _, _ in masters]
    all_have_audio = all(minfo["has_audio"] for _, minfo, _ in masters)
    print(f"    Combined highlight: ~{sum(e - s for _, s, e in high_segs):.0f}s | "
          f"combined edit: {sum(e - s for _, s, e in keep_segs):.0f}s")

    aspects = ["9x16", "16x9"] if args.aspect == "both" else [args.aspect]
    for aspect in aspects:
        tag = "vertical" if aspect == "9x16" else "horizontal"

        reel = os.path.join(out_dir, f"{base}_highlight_{tag}.mp4")
        print(f"    Rendering combined highlight reel ({tag})...")
        if render_segments(inputs, high_segs, aspect, reel, music, keep_audio=False):
            print(f"      Saved: {reel}")

        if not args.no_full:
            full = os.path.join(out_dir, f"{base}_edit_{tag}.mp4")
            print(f"    Rendering combined trimmed edit ({tag})...")
            if render_segments(inputs, keep_segs, aspect, full, None,
                               keep_audio=all_have_audio):
                print(f"      Saved: {full}")

    for m, _, _ in masters:
        if os.path.exists(m):
            os.remove(m)
...
def collect_videos(inputs: list) -> list:
    """Expand a mix of files and folders into an ordered list of video files."""
    videos = []
    for item in inputs:
        if os.path.isdir(item):
            videos += sorted(f for f in glob.glob(os.path.join(item, "*"))
                             if f.endswith(VIDEO_EXTS))
        elif os.path.isfile(item) and item.endswith(VIDEO_EXTS):
            videos.append(item)
        else:
            print(f"NOTE: skipping '{item}' (not a video file or folder)")
    return videos
...
def main():
    p = argparse.ArgumentParser(
        description="Auto-edit drone footage: trim dead parts, stabilize, "
                    "color-grade, and build music-backed highlight reels in "
                    "9:16 and 16:9. Pass one clip, or several with --combine "
                    "to merge them into a single video.")
    p.add_argument("input", nargs="+",
                   help="Video file(s) and/or folder(s) of drone footage")
    p.add_argument("--combine", action="store_true",
                   help="Merge all the given clips into ONE highlight reel "
                        "(and one combined trimmed edit)")
    p.add_argument("--name", default="combined",
                   help="Base name for the combined output (default: combined)")
    ...
    if not videos:
        sys.exit("No video files found in the given input(s).")
    ...
    if args.combine:
        print(f"\nMerging {len(videos)} clip(s) into one video → ./{args.out}/")
        process_combined(videos, args.out, args, stabilize, music, args.name)
    else:
        print(f"\nEditing {len(videos)} clip(s) individually → ./{args.out}/")
        for src in videos:
            try:
                process_single(src, args.out, args, stabilize, music)
